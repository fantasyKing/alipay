...
class Alipay {
  ...
  mobile_security_pay(data, key) {
    ...
    const params = {
      ...
      notify_url: url.resolve(this.config.host, this.config.notify.mobile_security_pay),
  ...
  create_direct_pay_by_user(data, key) {
    ...
    const parameter = {
      ...
      notify_url: url.resolve(this.config.host, this.config.notify.create_direct_pay_by_user), //服务器异步通知页面路径,必填，不能修改, 需http://格式的完整路径，不能加?id=123这类自定义参数,
      return_url: url.resolve(this.config.host, this.config.return.create_direct_pay_by_user), //页面跳转同步通知页面路径 需http://格式的完整路径，不能加?id=123这类自定义参数，不能写成http://localhost/,
  ...
  create_partner_trade_by_buyer(data, key) {
    ...
    const parameter = {
      ...
      partner: this.config.partner,
      payment_type: this.config.payment_type.buy,
      notify_url: url.resolve(this.config.host, this.config.notify.create_partner_trade_by_buyer),
      return_url: url.resolve(this.config.host, this.config.return.create_partner_trade_by_buyer),
  ...
  trade_create_by_buyer(data, key) {
    ...
    const parameter = {
      ...
      partner: this.config.partner,
      payment_type: this.config.payment_type.buy,
      notify_url: url.resolve(this.config.host, this.config.notify.trade_create_by_buyer),
      return_url: url.resolve(this.config.host, this.config.return.trade_create_by_buyer),
      seller_email: this.config.seller_email,
      out_trade_no: data.out_trade_no,
      subject: data.subject,
      price: data.price,
